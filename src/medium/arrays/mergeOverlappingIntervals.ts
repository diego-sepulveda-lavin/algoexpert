/* 
    ...
    Each interval "interval" is an array of two integers, with "interval[0] as the start of the interval and "interval[1]"" as the end
    of the interval.
...
export function mergeOverlappingIntervals(array: number[][]) {
  ...
  for (const nextInterval of sortedIntervals) {
    let [currIntervalStart, currIntervalEnd] = currentInterval;
    let [nextIntervalStart, nextIntervalEnd] = nextInterval;

    if (currIntervalEnd >= nextIntervalStart) {
      currentInterval[1] = Math.max(currIntervalEnd, nextIntervalEnd);
    } else {
